(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    module.exports = factory(require('./cube-engine.js'));
  } else {
    root.CubeSolver = factory(root.CubeEngine);
  }
...
//
// Colors are resolved from the fixed E.WHITE_DOWN_SCHEME, not live center
// pieces (a 2x2 has none): the first layer is always white, built on the
// bottom, with yellow finished on top — the usual white-first beginner frame.
// The displayed cube rests white-up (E.DEFAULT_SCHEME); with no fixed centers a
// 2x2 can be solved into any orientation, so no flip is needed. Verified
// against 3,000 scrambles from the white-up solved state: all solved, all
// finishing with white on the bottom.
//
...
function f1CornerDefs2x2() {
  const dColor = E.WHITE_DOWN_SCHEME[E.FACE_KEY.D];
  const slots = [['F', 'R'], ['R', 'B'], ['B', 'L'], ['L', 'F']];
  return slots.map(([s1, s2]) => {
    const c1 = E.WHITE_DOWN_SCHEME[E.FACE_KEY[s1]], c2 = E.WHITE_DOWN_SCHEME[E.FACE_KEY[s2]];
    const targetMap = {}; targetMap[dColor] = 'D'; targetMap[c1] = s1; targetMap[c2] = s2;
    return { colorSet: [dColor, c1, c2], targetMap, relevantFaces: [s1, s2], label: 'Corner (' + dColor + '/' + c1 + '/' + c2 + ')' };
  });
}
...
function llCornerDefs2x2() {
  const uColor = E.WHITE_DOWN_SCHEME[E.FACE_KEY.U];
  return [['F', 'R'], ['R', 'B'], ['B', 'L'], ['L', 'F']].map(([a, b]) => {
    const c1 = E.WHITE_DOWN_SCHEME[E.FACE_KEY[a]], c2 = E.WHITE_DOWN_SCHEME[E.FACE_KEY[b]];
    const targetMap = {}; targetMap[uColor] = 'U'; targetMap[c1] = a; targetMap[c2] = b;
    const slot = [E.FACE_POS[a][0] + E.FACE_POS[b][0], 1, E.FACE_POS[a][2] + E.FACE_POS[b][2]];
    return { colorSet: [uColor, c1, c2], targetMap, slot, label: 'Last-layer corner (' + uColor + '/' + c1 + '/' + c2 + ')' };
  });
